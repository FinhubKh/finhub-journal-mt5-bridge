...
def deals_to_trades(deals: list) -> list:
    ...
    out = []
    for _pos_id, group in by_pos.items():
        entry = next((d for d in group if d.get("entry") == "in"), None)
        exit_ = next((d for d in group if d.get("entry") == "out"), None)
        if not exit_:
            continue
        exit_type = (exit_.get("type") or "").lower()
        direction = "buy" if exit_type == "sell" else "sell"
        pnl_raw = float(exit_.get("profit") or 0) + float(exit_.get("swap") or 0) + float(exit_.get("commission") or 0)
        if entry:
